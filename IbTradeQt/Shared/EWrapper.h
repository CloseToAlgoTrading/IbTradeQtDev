...
public:
   virtual ~EWrapper() {};
    ...
    virtual void updateMktDepthL2(TickerId id, int position, const std::string& marketMaker, int operation,
        int side, double price, int size, bool isSmartDepth) =0;
    ...
    virtual void historicalData(TickerId reqId, const Bar& bar) =0;
    virtual void historicalDataEnd(int reqId, const std::string& startDateStr, const std::string& endDateStr) =0;
    ...
    virtual void deltaNeutralValidation(int reqId, const DeltaNeutralContract& deltaNeutralContract) =0;
    ...
    virtual void securityDefinitionOptionalParameter(int reqId, const std::string& exchange, int underlyingConId, const std::string& tradingClass,
        const std::string& multiplier, const std::set<std::string>& expirations, const std::set<double>& strikes) =0;
    ...
    virtual void smartComponents(int reqId, const SmartComponentsMap& theMap) =0;
    virtual void tickReqParams(int tickerId, double minTick, const std::string& bboExchange, int snapshotPermissions) =0;
    ...
    virtual void histogramData(int reqId, const HistogramDataVector& data) =0;
    virtual void historicalDataUpdate(TickerId reqId, const Bar& bar) =0;
    ...
    virtual void tickByTickAllLast(int reqId, int tickType, time_t time, double price, int size, const TickAttribLast& tickAttribLast, const std::string& exchange, const std::string& specialConditions) =0;
    virtual void tickByTickBidAsk(int reqId, time_t time, double bidPrice, double askPrice, int bidSize, int askSize, const TickAttribBidAsk& tickAttribBidAsk) =0;
    virtual void tickByTickMidPoint(int reqId, time_t time, double midPoint) =0;
    virtual void orderBound(long long orderId, int apiClientId, int apiOrderId) =0;
};
